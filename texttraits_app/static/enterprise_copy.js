(function () {
  ...
  function subjectLines(context) {
    ...
    return [
      `The two accounts to coach first`,
      `Before ${compactPhrase(context.trigger, 4)} slows the team`,
      `Cleaner notes before forecast review`,
      `${context.company} x {{company}}`,
      `A quick read on ${pain}`,
    ];
  }
  ...
  function buildEmailVariant(context, variant, options = {}) {
    ...
    const learnedLine = learned.includes("Proof before product")
      ? "I would lead with one proof point before any product detail."
    ...
    const tonePrefix = variant === "A"
      ...
      : variant === "B"
        ? `This does not sound like they need another dashboard. It sounds like ${compactPhrase(context.trigger, 5)} is making it harder to know which account deserves coaching first.`
        : `For ${context.role}, the useful hook is a cleaner prep note before the next account review.`;
    const proof = variant === "A" ? context.proof : variant === "B" ? context.caseStudy : context.competitor;
    const body = variant === "A"
      ? `${tonePrefix}

If that is the pressure right now, I would anchor the email to one job: help managers see which two accounts deserve coaching before the next review.

${context.company} helps ${context.segment} teams turn ${context.trigger} into a short list of named accounts, why each account matters, and the rep note a manager should review before the meeting. The proof point to anchor on is ${proof}.${preferSpecific ? ` The first test could focus on ${context.icp}.` : ""}
...
${body}`;
  ...
  function buildSequence(context) {
    return [
      ["Day 1", "Initial email", `Subject: ${subjectLines(context)[0]}\n\nHi {{first_name}},\n\nI noticed the team is focused on ${context.pain}. If the goal is to spot the two accounts that need coaching before forecast review, I had a short idea tied to ${context.trigger}.\n\n${ctaText(context)}`],
      ...
      ["Day 6", "Proof follow-up", `One proof point that may be relevant: ${context.proof}. The useful shift is usually a cleaner manager note before the team spends another meeting guessing.`],
  ...
  function inboxReply(thread, context) {
    ...
    if (thread.type === "interested") return `Hi ${firstName}, glad that is the key question. The short version: you can test this with one team and one review cycle before any heavy migration. Worth a focused 15-minute fit call next week?`;
    if (thread.type === "objection") return `Hi ${firstName}, that makes sense. I would not pitch this as another dashboard. The useful angle is one cleaner manager note before forecast review. Want me to send a two-bullet example?`;
