...
def main() -> None:
    ...
    assert_true("starter-sample-panel" not in app_js, "Enterprise setup should avoid duplicate starter sample panels")
    assert_true('enterprise-sample-drawer" ${state.sampleWorkspaceLoaded ? "open" : ""}' in app_js, "Loaded Enterprise samples should keep their next action visible")
    assert_true("state.batchInput = sampleCsv;" in app_js and "Refresh sample data" in app_js, "Enterprise sample workspace should populate queues instead of only showing a toast")
    assert_true("habit-strip" in app_js and "habit-strip" in css, "Explorer result should expose calm journal progress")
    assert_true("daily-home-card" in app_js and "streakMilestone" in app_js, "Explorer empty state needs a daily home and streak milestone")
    assert_true("inferExplorerSource" in app_js and "goalRewriteClose" in app_js, "Explorer rewrites should adapt to source type and goal")
    ...
    assert_true("enterpriseFocusMode" in app_js and "Focus editor" in app_js, "Enterprise draft editor needs a focused review mode")
    assert_true("lastDraftChange" in app_js and "What changed" in app_js, "Enterprise transforms should explain what changed")
    ...
    assert_true("accountCodePanelOpen" in app_js and "account-code-panel" in css, "Verification/reset code UI should remain reachable inside the account sheet")
    assert_true("trapAccountFocus" in app_js and 'event.key === "Tab"' in app_js, "Account modal should trap keyboard focus")
    assert_true("explorerStyleDetailsOpen" in app_js and "explorerQuickChecksOpen" in app_js, "Explorer expanded sections should not collapse after tab clicks")
    assert_true("integrationSetupOpen" in app_js, "Enterprise CRM setup should land on visible setup requirements")
    assert_true('data-generate-batch ${canGenerate' not in app_js, "Batch generation should not disable away its own empty-input guidance")
