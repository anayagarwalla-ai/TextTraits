...
def main() -> int:
    ...
    assert_true("--motion-fast: 220ms" in styles_css and "--motion-soft: 360ms" in styles_css, "Click motion should be paced slowly enough to read")
    ...
    assert_true(".quiet-more-drawer[open]" in styles_css and "gap: 18px" in styles_css, "More drawer should keep visible spacing between stacked boxes")
    ...
    assert_true(".rewrite-card .result-actions" in styles_css and "margin-top: 10px" in styles_css, "Rewrite action buttons should not crowd the rewrite text")
