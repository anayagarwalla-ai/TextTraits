...
def main() -> int:
    ...
    assert_true("--motion-fast: 420ms" in styles_css and "--motion-soft: 680ms" in styles_css, "Click motion should be paced slowly enough to read")
    assert_true("message-mode-detail" in app_js and "message-mode-detail" in styles_css and "margin-top: 2px" in styles_css, "Message Coach mode copy should stay in one consistent below-chips position")
    ...
    assert_true(".quiet-more-drawer > .muted + .field" in styles_css and "margin-top: 18px" in styles_css, "More drawer helper text should not crowd the sample-name field label")
    ...
    assert_true("function rewriteTitle" in app_js and "Clearer version" in app_js, "Rewrite title should not read like 'Make this clearer version'")
    assert_true("withTerminalPunctuation" in app_js, "Rewrite preview copy should add missing terminal punctuation")
    assert_true("Make this clearer version" not in app_js, "Ungrammatical rewrite title leaked into Explorer output")
    assert_true(".quiet-more-drawer .field-grid" in styles_css and "gap: 28px 30px" in styles_css and ".start-card strong" in styles_css, "Todo spacing fixes should cover More controls and enterprise start card")
