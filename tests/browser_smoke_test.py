...
def main() -> int:
    ...
    for route_html in (html, explorer_html, enterprise_html):
        ...
        assert_true("Skip to analysis workspace" in route_html and "Local build:" in route_html, "accessible local build shell missing")
    ...
    assert_true("TextTraitsApi" in api_js and "evaluate" in api_js and "analyzeEmail" in api_js and "b2bProfile" in api_js, "API client global missing")
    ...
    assert_true(".b2b-profile-builder" in css and ".b2b-profile-report" in css and ".profile-playbook" in css, "B2B profile styling missing")
    assert_true(".hash-line" in css and "overflow-wrap: anywhere" in css, "hash overflow protection missing")
    assert_true(".enterprise-nav" in css and "position: static;" in css, "transparent nav should not overlap scrolled content")
