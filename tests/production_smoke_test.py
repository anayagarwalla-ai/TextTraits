...
def main() -> int:
    ...
    save = client.put("/api/workspace", json={"name": "QA workspace", "data": workspace_payload})
    ...
    event = client.post("/api/events", json={"event_type": "qa_event", "payload": {"ok": True}})
    assert_true(event.status_code == 200, "event logging failed")

    client_error = client.post("/api/client-errors", json={"message": "qa client error"})
    ...
    oauth_start = client.get("/api/integrations/hubspot/oauth/start")
    ...
    reset_request = client.post("/api/request-password-reset", json={"email": "qa@example.com"})
    ...
    reset = client.post("/api/reset-password", json={"token": token, "password": "new-texttraits-test"})
    ...
    login = client.post("/api/login", json={"email": "qa@example.com", "password": "new-texttraits-test"})
    assert_true(login.status_code == 200, "login after reset failed")
    delete = client.delete("/api/account")
    ...
    empty = client.post("/evaluate", json={"text": ""})
    assert_true(empty.status_code == 400, "empty input should fail")
    too_long = client.post("/evaluate", json={"text": "word " * 1905})
